...
**Root Directory**를 `apps/public-next`, `apps/admin` 처럼 앱 단위로 이미 설정해 두었다면 그대로 두면 됩니다.

### 설정 방법 (둘 중 하나)

1. **코드로 설정 (권장)**  
   각 앱에 `vercel.json`에 `ignoreCommand`가 들어 있어서, **대시보드에서 따로 넣을 필요 없습니다.**  
   - `apps/public-next/vercel.json`  
...
|----------|-------------------------|
| **public-next** (Next 공개 앱) | `bash scripts/vercel-ignore-build.sh apps/public-next packages/shared` |
| **admin** (어드민) | `bash scripts/vercel-ignore-build.sh apps/admin packages/shared` |
| **public** (Vite 공개 앱, 사용 시) | `apps/public/vercel.json` 추가 시 `ignoreCommand`에 `cd ../.. && bash scripts/vercel-ignore-build.sh apps/public packages/shared` |
...
- **Build Filters**: **Included Paths**에 넣은 경로가 바뀔 때만 배포. 경로는 **항상 저장소 루트 기준**입니다.  
  → API 서비스는 `apps/api/**` 와 `packages/shared/**` 만 포함하면 됩니다.
